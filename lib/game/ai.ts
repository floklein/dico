...
export async function normalizeAndFillDefinitions(
  ...
  const json = await callGatewayJson<{ definitions: DefinitionBatchOutput[] }>(
    [
      ...
      "Si rawDefinition est null, invente une fausse définition plausible mais totalement hors-sujet, sans rapport avec le mot ni sa vraie définition.",
      "Interdiction absolue de donner un indice sémantique, un synonyme, une étymologie, un domaine ou une explication partiellement vraie du mot.",
      "La fausse définition doit rester crédible comme phrase de joueur pressé : naturel, imparfait, parfois fragment, pas toujours une phrase complète.",
  ...
  if (!Array.isArray(json.definitions)) {
    throw new Error("Sortie IA invalide : les définitions doivent être un tableau.");
  }
